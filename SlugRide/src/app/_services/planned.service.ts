...
export class PlannedService {
    ...
    approveRider(driver_email : string, rider_email : string, trip_id : string, rider_approval : boolean) {
        return this.http.post('http://localhost:8000/rideshare/get_riders_on_trip/', JSON.stringify({driver_email : driver_email, rider_email : rider_email, trip_id : trip_id, rider_approval : rider_approval})).map((response: Response) => response.json());
    }
